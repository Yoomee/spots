...
(function($){
	$.fn.faceList = function(data, options) {
		var defaults = { 
			...
			selectedItem: 		"name", //name of object property
			selectedValues: 	"value", //name of object property
			searchObj:		 	"name", //comma separated list of object property names
			queryParam: 		"q",
		...
		if((data_type == "object" && data_count > 0) || data_type == "string"){
			return this.each(function(x){
						...
						case 8:  // delete
							if(input.val() == ""){							
								...
								if(org_li.prev().hasClass("selected")){
									...
									opts.itemRemoved.call(org_li.prev());
									org_li.prev().remove();
								} else {
                ...
                // add_selected_item({"name":input.val(), "value":input.val()}, 0);
                // input.val("");
                // input.focus();
                if(opts.neverSubmit){
                  e.preventDefault();
				...
				function processDataQuery(data, query){
					...
					for(var i=0;i<data_count;i++){				
						...
						if(str){
							...
							//Don't hide results that don't match query term, trust controller.
					...
					results_ul.css("width", selections_holder.outerWidth());
					results_holder.show();
          var first_result = $("li",results_holder).eq(0);
          first_result.addClass("active");
					opts.resultsComplete.call(this);
				}
				
				function add_selected_item(data, num){
				  if (data[opts.selectedItem] == "") {
  				  return false;
				  }
					...
					org_li.before(item.html(data[opts.selectedItem]).append(close));
				}
