...
CKEDITOR.dialog.add( 'link', function( editor )
{
	...
	var protocolChanged = function() {
	  var curLink = $('#linkUrl').html();
	  $('#linkUrl').html(curLink.replace(/^\w+:\/\//, this.getValue()));
	};
		...
		contents : [
		  {
	        ...
	        {
	          type: 'html',
	          html: "<p>Please begin typing a page or item which you wish to link to,<br /> and select the item you would like.</p>"
	        },
		      {
		        id: 'searchTerm',
		        type: 'text',
		        label: 'Search'/*,
		        onKeyUp: searchTermChanged*/
		      },
		      {
		        id: 'linkUrl',
		        type: 'html',
		        html: "<div id='linkUrlContainer'>Link: <span id='linkUrl'></span></div>"
		      },
		      {
		        type: 'html',
...
/*			{
		...
		onOk : function()
		{
			...
			if ( data.target )
			{
				...
				{
					if ( data.target.type != 'notSet' && data.target.type)
		...
		onLoad : function()
		{
		  var dialog = this.definition.dialog;
		  var linkTypeEl = dialog.getContentElement('info', 'linkType');
		  var protocolEl = dialog.getContentElement('info', 'protocol');
		  var urlEl = dialog.getContentElement('info', 'url');
		  $('#' + this.getContentElement('searchSite', 'searchTerm').domId + ' input').autocomplete(
		    '/search/jquery_autocomplete.json',
		    {
  		    dataType: 'json',
  		    parse: function(data) {
  		      var result = [];
  		      for (var i = 0; i < data.length; i++) {
  		        result[result.length] = {data: data[i], value: data[i]};
  		      }
            return result;
  		    },
		    ...
		    // Set preview label
        // Set values on info tab
        var result_name = data.name;
        $(this).val(result_name.replace(/<\/?em>/g, ''));
        linkTypeEl.setValue('url');
        protocolEl.setValue('http://');
        urlEl.setValue(data.url.replace(/^http:\/\//, ''));
        $('#linkUrl').html(data.url);
		  });
