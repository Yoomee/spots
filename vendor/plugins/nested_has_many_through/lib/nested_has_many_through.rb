module NestedHasManyThrough
  ...
  module Associations
    def self.included(base)
      base.module_eval do
        valid_keys_for_has_many_association << :nested if valid_keys_for_has_many_association
        valid_keys_for_has_one_association << :nested if valid_keys_for_has_one_association
      end
    end
  end
  ...
  module Association
    def self.included(base)
      base.class_eval do
        def construct_conditions_with_nested
          if @reflection.through_reflection && @reflection.options[:nested]
   ...
   #"#{@nested_join_attributes[:remote_key]} = a #{@nested_join_attributes[:conditions]}"
        ...
        alias_method_chain :construct_conditions, :nested

        def construct_joins_with_nested(custom_joins = nil)
          if @reflection.through_reflection && @reflection.options[:nested]
            @nested_join_attributes ||= construct_nested_join_attributes
            "#{@nested_join_attributes[:joins]} #{custom_joins}"
          else
            construct_joins_without_nested(custom_joins)
          end
        end
        alias_method_chain :construct_joins, :nested
      end
    end
