module FancyBox
  ...
  def link_to_box(content, link, options={})
    ...
    if wrapper_class = options.delete(:fancybox_wrapper_class)
      options[:onclick] ||= ""
      options[:onclick] = "$('#fancybox-wrap').attr('class', '#{wrapper_class}');" + options[:onclick]
    end
    ...
    options[:class] << " iframe" if (!options.delete(:inline) && determine_request(link) == :remote)
    options[:id] = elem_id
  ...
  def link_to_image(src,options={})
    ...
    link_to image_tag(src, options.delete(:image_options)), link_src, options
  end
  ...
  protected
    ...
    def determine_request(req)
      ...
      return :remote if req =~ /^[^\/]/ #TLDS.match(req)
      return :standard
    end
